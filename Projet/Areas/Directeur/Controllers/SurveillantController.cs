    ...
    public class SurveillantController : Controller
    {
        ...
        public async Task<IActionResult> CalculChargeSurveillant(int? IdS = null, int pg = 1, int? EId = null, int? VId = null, int? DId = null ,int? GId = null)
        {
            ...
            if (DId!= null)
            {
                ...
                {
                    enseignants = _context.Enseignants.Include(e => e.departement).Include(g => g.gradeEnseigant).Where(e => e.IdEnseignant == EId && e.IdDepartement == DId).ToList();
                    vacataires = _context.vacataires.Include(v => v.departement).Include(g => g.gradeEnseigant).Where(e => e.IdVacataire == VId && e.IdDepartement == DId).ToList();
                }
                else if (GId != null)
                {
                    enseignants = _context.Enseignants.Include(e => e.departement).Include(g => g.gradeEnseigant).Where(e => e.IdDepartement == DId && e.GradeId == GId).ToList();
                    vacataires = _context.vacataires.Include(v => v.departement).Include(g => g.gradeEnseigant).Where(e => e.IdDepartement == DId && e.GradeId == GId).ToList();
                }
                else { 
                enseignants = _context.Enseignants.Include(e => e.departement).Include(g=>g.gradeEnseigant).Where(e => e.IdDepartement == DId).ToList();
            
                 vacataires = _context.vacataires.Include(v => v.departement).Include(g=>g.gradeEnseigant).Where(e => e.IdDepartement == DId).ToList();
                }
            }
            else if (VId.HasValue || EId.HasValue )
            {
                enseignants = _context.Enseignants.Include(e => e.departement).Include(g => g.gradeEnseigant).Where(e => e.IdEnseignant == EId).ToList();
                vacataires = _context.vacataires.Include(v => v.departement).Include(g => g.gradeEnseigant).Where(e => e.IdVacataire == VId).ToList();

            }
            else if (GId != null)
            {
                enseignants = _context.Enseignants.Include(e => e.departement).Include(g => g.gradeEnseigant).Where( e=> e.GradeId == GId).ToList();
                vacataires = _context.vacataires.Include(v => v.departement).Include(g => g.gradeEnseigant).Where( e=>e.GradeId == GId).ToList();
            }


            else
            {
                enseignants = _context.Enseignants.Include(e => e.departement).Include(g=>g.gradeEnseigant).ToList();
                vacataires = _context.vacataires.Include(v => v.departement).Include(g => g.gradeEnseigant).ToList();
            }
